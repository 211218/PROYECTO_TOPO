...
type Topos struct {
	...
	status  bool
	topo    *canvas.Image
	pelota1 *Ball // Agrega una referencia a la primera pelota
	pelota2 *Ball // Agrega una referencia a la segunda pelota
	pelota3 *Ball // Agrega una referencia a la tercera pelota
	topoWin *TopoWin
}

func NewTopos(posX float32, posY float32, img *canvas.Image, pelota1 *Ball, pelota2 *Ball, pelota3 *Ball, topoWin *TopoWin) *Topos {
	return &Topos{
		posX:    posX,
		posY:    posY,
		status:  true,
